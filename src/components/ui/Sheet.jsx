...
const SheetContent = React.forwardRef(
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      {/* <SheetOverlay /> */}
      <div
        className="fixed inset-0 bg-black/80 bg-opacity-75 transition-opacity"
        aria-hidden="true"
      ></div>
      ...
      >
        ...
        {children}
        <SheetPrimitive.Close className="absolute right-6 top-6 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-transparent">
          <X className="h-5 w-5" />
